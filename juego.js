...
function isTheGameOver(){
  let color1 = piezas[0].color;
  for (let i = 0; 0 < piezas.length; i++) {
    if (color1 !== piezas[i].color) {
      //FALSE	
      return false;
    }
  }
...
function isThereAPieceBetween(casilla1, casilla2) {
  ...
  let x = Math.floor((casilla1.column + casilla2.column) / 2);
  let y = Math.floor((casilla1.row + casilla2.row) / 2);
  let heburu = aurkituPieza(x, y);

  return thereIsPiece(helburu);
}

function aurkituPieza(x, y) {

  console.log("Aurkitu beharreko piezaren koordenatuak x:" + x + " eta y:" + y);
...
function thereIsPiece(casilla) {
  if (!isNaN(aurkituPieza(casilla.column, casilla.row))) {
    //true
    return true
  } else {
    //false	
